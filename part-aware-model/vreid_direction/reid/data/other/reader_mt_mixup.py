...
from . transform import build_transform_mixup
...
class ReaderMT(object):
    ...
    def __init__(self,
        ...
        # transform
        assert cfg!=None
        train_transform, test_transform = build_transform_mixup(cfg)
        if is_test:
        ...
        if sample_type == 'Identity':
            ...
            self.index_sampler_mixup = RandomIdentitySampler(self._roidbs, self._num_batch_pids, self._num_instances)
            ...
            self.index_sampler_mixup = BaseSampler(self._roidbs, self._shuffle)
        ...
        assert self._sample_num == len(self.index_sampler_mixup)
        #pdb.set_trace()
    ...
    def reset(self):
        ...
        self.indexes_mixup = self.index_sampler_mixup.reset()
    ...
    def _load_batch(self):
        ...
        while bs != self._batch_size:
            ...
            pos_mixup = self.indexes_mixup[self._pos]
            ...
            if self.use_mixup:
                sample['mixup'] = copy.deepcopy(self._roidbs[pos_mixup])
                if self._load_img:
