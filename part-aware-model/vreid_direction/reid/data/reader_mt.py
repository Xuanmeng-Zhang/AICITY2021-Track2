...
class RandomIdentitySamplerV2(object):
    def __init__(self, data_source, num_batch_pids=16, num_instances=1):
        self.data_source = data_source
        self.batch_size = num_batch_pids * num_instances
        self.num_batch_pids = num_batch_pids
        self.num_instances = num_instances
        self.index_dic = defaultdict(list)
        self.num_images = len(data_source)
        ...
        for pid in self.pids:
            ...
            cur_num = len(idxs)
            if cur_num < self.num_instances:
                cur_num = self.num_instances
            self.length += cur_num - cur_num % self.num_instances
        num_samples = len(self.pids)
        self.index_list = np.arange(num_samples)
        self.num_last_pids = num_samples % self.num_batch_pids
        self.num_samples = num_samples - self.num_last_pids
        self.num_iters_per_epoch = self.length / float(self.batch_size)
    ...
    def reset(self):
        ...
        final_idxs = []
        while len(avai_pids) >= self.num_batch_pids:
            selected_pids = random.sample(avai_pids, self.num_batch_pids)
...
class ReaderMT(object):
    ...
    def __init__(self,
                 ...
                 bufsize=100,
                 memsize='6G',
#                  input_fields=['image','camid','colorid', 'typeid','target_size'],
                 input_fields = ['image',"orientation",'target_size'],
                 is_test = False,
                 flip = False,
                 cfg=None):
...
#         if not flip:
#             print("using build_transform!!!")
#             train_transform, test_transform = build_transform(cfg)
#         else:
#             print("using build_transform_flip!!!")
#             train_transform, test_transform = build_transform_flip(cfg)
        ...
        elif sample_type == 'IdentityV2':
            print('Using V2 Identity sampler')
            assert self._shuffle == True
            assert self._drop_last == True
            self.index_sampler = RandomIdentitySamplerV2(self._roidbs, self._num_batch_pids, self._num_instances)
            self.num_classes = self.index_sampler.num_classes
            self.num_iters_per_epoch = self.index_sampler.num_iters_per_epoch
            self.num_batch_pids = self.index_sampler.num_batch_pids
        else:
            self.index_sampler = BaseSampler(self._roidbs, self._shuffle)
            self.num_classes = 1557
            self.num_iters_per_epoch = len(self.index_sampler) 
            self.num_batch_pids = self._batch_size 
        # sampling
    ...
    def worker(self, batch_samples=None):
        ...
        """
        batch = []
        selected_size = random.choice(cfg.target_size)
        
        for sample in batch_samples:
            sample["target_size"] = selected_size
