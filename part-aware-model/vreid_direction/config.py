...
def parse_args():
    ...
    #add_arg('model_arch',       str,    'ResNet50_vd',     "The model arch to train.")
    #add_arg('model_arch',       str,    'HRNet_W48_C',     "The model arch to train.")
    add_arg('model_arch',       str,    'ResNet101_vd',     "The model arch to train.")
    add_arg('model_save_dir',   str,    'out_ResNet101_vd',     "The path to save model.")

    add_arg('pretrain',         str,    'pretrained/ResNet101_vd_ssld_pretrained', "The pretrain model path.")
    add_arg('weights',          str,    'model_final', "The weights path.")

    add_arg('data_dir',         str,    'dataset/aicity_data',      "The data root path.")

    #SOLVER
    #add_arg('batch_size',       int,    32,     "Mini-batch size per device.")
    add_arg('batch_size',       int,    4,     "Mini-batch size per device.")
    ...
    add_arg('warm_up_iter',     float,  2000,  "Learning rate.")
    add_arg('start_iter',       int,    0,      "Start iteration.")
    add_arg('max_iter',         int,    30000, "Iter number.")    
    add_arg('snapshot_iter',    int,    1000,   "Save model every snapshot stride.")
    parser.add_argument('--lr_steps', nargs='+', type=int, default=[16000, 24000, 28000], help="The mean of input image data")
    #parser.add_argument('--lr_steps', nargs='+', type=int, default=[100000, 160000, 200000], help="The mean of input image data")


    add_arg('margin',     float,  0.3,  "intra class margin for IntraLoss.")
    ...
    add_arg('target_size',     list,    [384],    "Image target_width.")
