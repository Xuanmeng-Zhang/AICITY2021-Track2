...
def main(cfg):
    ...
    #cls_out, fea_out = model.net(input=image, is_train=False, class_dim=273, num_features = cfg.num_features)
    #cls_out, fea_out = model.net(input=image, is_train=False, class_dim=1535, num_features = cfg.num_features)
    #cls_out, fea_out = model.net(input=image, is_train=False, class_dim=1617, num_features = cfg.num_features)
    #cls_out, fea_out = model.net(input=image, is_train=False, class_dim=333, num_features = cfg.num_features)
    #cls_out, fea_out = model.net(input=image, is_train=False, class_dim=1695, num_features = cfg.num_features)
    cls_out, fea_out = model.net_orientation(input=image, is_train=False, class_dim=8, num_features = cfg.num_features)
    index = fluid.layers.cast(index, dtype='int32') 
    fea_out = fluid.layers.l2_normalize(x=fea_out, axis=1)

    pred_list = [fea_out, index,cls_out]
    test_prog = fluid.default_main_program().clone(for_test=True)
    image.persistable = True
    fea_out.persistable = True
    cls_out.persistable = True
    ...
    reader_config = {'dataset':query_source, 
                     'img_dir':'./dataset/aicity_data/image_query',
    ...
    for data in data_loader():
        ...
        pred_direction = out[2]
        #cur_image = out[2]
        #pdb.set_trace()
        count = count+ len(cur_index)
        for single_index, fea in zip(cur_index, pred_direction):
            #print(single_index)
            fname = query_names[single_index]
#             query_dict[fname] = direction
    ...
    reader_config = {'dataset':gallery_source, 
                     'img_dir':'./dataset/aicity_data/image_test',
                     'batch_size':cfg.test_batch_size,
                     'num_instances':cfg.num_instances,
                     'is_test':True,
                     'sample_type':'Base',
