...
from . transform import build_transform, build_transform_flip
...
def StandardizeDataset(dataset, root='./downsample_vehicle/images'):
...
class RandomIdentitySamplerV2(object):
    ...
    def __init__(self, data_source, num_batch_pids=16, num_instances=1):
        ...
        # this length maynot divide the total len
        self.num_iters_per_epoch = int(self.length / self.batch_size)
...
class Reader(object):
    ...
    def __init__(self,
        ...
        if not isinstance(dataset[0],dict):
            self._roidbs = StandardizeDataset(dataset, img_dir)
        else:
