...
#                shape=[1], dtype='float32', value=float(values[i]))
#            with switch.case(global_step < boundary_val):
#                cur_epoch_factor = (global_step - boundaries[i]) / boundaries[i]
#                pdb.set_trace()
#                cur_epoch_cos_factor = math.cos( cur_epoch_factor * math.pi / 2.0 )
#                cur_lr = values[i] * cur_epoch_cos_factor
#
#                fluid.layers.assign(value_var, cur_lr)
#
#        last_value_var = fluid.layers.fill_constant(
#            shape=[1], dtype='float32', value=float(values[len(values) - 1]))
#        with switch.default():
