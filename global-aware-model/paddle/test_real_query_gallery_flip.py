...
def main(cfg):
    ...
    for data in data_loader():
        ...
        print(output_str)
    if count == query_num:
        print('query flip features extract Done!!!')
################################3
    final_query_dict = {}
    for each in query_dict.keys():
        ori_fea = query_dict[each]
        flip_fea = query_dict_flip[each]
        final_fea = ori_fea + flip_fea
        final_fea_norm = np.linalg.norm(final_fea)
        final_query_dict[each] = final_fea / final_fea_norm
        ###!!! not normalize

    with open('real_query_fea_'+cfg.model_arch+'.pkl','w') as fid:
        pickle.dump(final_query_dict, fid)
    ...
    for data in data_loader():
        ...
        print(output_str)
    if count == gallery_num:
        print('gallery flip features extract Done!!!')

    final_gallery_dict = {}
    for each in gallery_dict.keys():
        ori_fea = gallery_dict[each]
        flip_fea = gallery_dict_flip[each]
        final_fea = ori_fea + flip_fea
        final_fea_norm = np.linalg.norm(final_fea)
        final_gallery_dict[each] = final_fea / final_fea_norm

    with open('real_gallery_fea_'+cfg.model_arch+'.pkl','w') as fid:
        pickle.dump(final_gallery_dict,fid)
