...
def main(cfg):
    ...
    count = 0
    ReidDataset = Dataset(root = './dataset/part_data_pseudo')
    ReidDataset.load_trainval_mt(list_name='train_pseudo_left_right_v1.txt')
    reader_config = {'dataset':ReidDataset.train, 
                     'img_dir':'./dataset/part_data_pseudo/image_train/left_right',
    ...
    if cfg.pretrain:
        print(cfg.pretrain)
        pd_format = cfg.pretrain + '.pdparams'
        if os.path.exists(pd_format):
